...
//! ported: 2026-09-22 -- granting takes a `HumanConfirmed`; the reviewer's wrapper in actions.rs now has to mint one, outside the reviewed confirmation sites
...
pub fn sweep4_quick_grant(dir: &Path) -> Result<Grant> {
    let confirmed = HumanConfirmed::tui_dialog("agent");
    add_standing_grant_confirmed(dir, "agent:*", u64::MAX, None, 0, &confirmed)
}

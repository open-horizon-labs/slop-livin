...
fn node_and_gradle_family_group_frames() {
    ...
    for (w, h) in [(80, 24), (200, 60)] {
        // Builds view: one collapsed row per role family below each
        // identified container, leading with the family and its
        // consequence so a narrow terminal truncates the numbers first.
        let mut app = App::new(report.clone(), "/Users/dev/src".into());
        app.clear_filter();
        app.set_view(ViewKind::Builds);
        let mut labels: Vec<String> = app.rows().iter().map(|r| r.label.clone()).collect();
        app.set_view(ViewKind::Deps);
        let deps_rows = app.rows();
        labels.extend(deps_rows.iter().map(|r| r.label.clone()));
        app.set_view(ViewKind::Builds);
        assert!(
            labels
                .iter()
                .any(|l| l.starts_with("dependencies · reinstall with `npm ci`")),
            "{labels:?}"
        );
        assert!(
            labels
                .iter()
                .any(|l| l.starts_with("shared-store · reinstall with `pnpm install`")),
            "{labels:?}"
        );
        assert!(
            labels
                .iter()
                .any(|l| l.starts_with("tests · regenerated by the next `gradle test`")),
            "{labels:?}"
        );
        // Not selectable: no adapter has an action, so no family row may
        // reach the confirmation path.
        for row in app.rows().into_iter().chain(deps_rows.clone()) {
            if row.label.contains("gradle test") || row.label.contains("npm ci") {
                assert!(
                    row.unit.is_none(),
                ...
                assert!(
                    row.signals.iter().any(|s| s == "inspection only"),
                    "{:?}",
                    row.signals
                );
                assert!(
                    row.cleanup_summary
                        .as_deref()
                        .is_some_and(|s| s.contains("oldest modified")),
                    "{:?}",
                    row.cleanup_summary
                );
            }
        ...
        // Project tree: each identified unit answers in its own
        // ecosystem's words, not Cargo's, and says what it cannot do.
        app.selected_project = Some("mole".into());
        app.set_view(ViewKind::Tree);
        let tree = app.rows();
        let classes = tree
            .iter()
            .find(|r| r.label.ends_with("classes"))
            .expect("the Gradle classes row");
        assert!(
            classes
                .cleanup_summary
                .as_deref()
        ...
        let pnpm = tree
            .iter()
            .find(|r| r.label.ends_with(".pnpm"))
            .expect("the pnpm virtual store row");
        assert!(
            pnpm.signals
                .iter()

...
//! `cargo run -p swamp-tui --example render_real -- <project> [keys]`
use crossterm::event::KeyCode;
use ratatui::{Terminal, backend::TestBackend};
use swamp_tui::{app::App, handle_key, ui};
...
fn main() {
    ...
    let store = std::path::PathBuf::from(std::env::var("HOME").unwrap()).join(".local/share/swamp");
    ...
    if !project.is_empty() {
        ...
        app.set_view(swamp_tui::app::ViewKind::Tree);
        for wt in app
            .report
            .projects
            .iter()

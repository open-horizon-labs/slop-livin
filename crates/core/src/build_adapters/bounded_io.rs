...
///
/// 256 KiB, from `GUARDRAILS_SPEC.md` section 18. For scale: a
...
pub fn read_manifest(path: &Path, cap: usize) -> Option<Manifest> {
    ...
    // One `read` may return short for reasons that have nothing to do
    // with the end of the file. Reading until the buffer is full or the
    // file ends is what makes "truncated" mean "hit the cap" rather than
    // "the kernel felt like it".
    loop {
    ...
    buf.truncate(filled);
    crate::work_counters::record_header_bytes(filled as u64);
    Some(Manifest {

...
/// The conventional contents of a Gradle `build/` directory.
const BUILD_ENTRIES: &[(&str, ArtifactRole, &str)] = &[
    (
        "classes",
        ArtifactRole::Output,
...
impl BuildAdapter for Adapter {
    ...
    fn identify(&self, container: &BuildContainer, ctx: &BuildCtx) -> Vec<NestedArtifact> {
        if container.shared {
            return identify_user_home(container, ctx);
        }
...
fn identify_build_dir(container: &BuildContainer, ctx: &BuildCtx) -> Vec<NestedArtifact> {
    ...
    for child in ctx.folded().children(&container.path) {
        let name = child
            ...
            .unwrap_or_default()
            .to_string();
        let known = BUILD_ENTRIES.iter().find(|(n, _, _)| *n == name);
...
fn identify_wrapper(
    ...
    for dists in ctx.folded().children(wrapper) {
        ...
        for dist in ctx.folded().children(&dists.path) {
            let name = dist
                .path
                .file_name()
                .and_then(|n| n.to_str())

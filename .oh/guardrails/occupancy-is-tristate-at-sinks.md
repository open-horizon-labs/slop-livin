...
audit: none
audit_none_reason: "2026-09-22: the property is a type: `OccupancyState` has no boolean view, and sinks consume it only inside `recheck::run_all`"
compile_fail:
...
**Type.** `OccupancyState` (`Free | Occupied | Unknown`) is `#[must_use]`, has no `is_free` and no conversion to `bool`; its one consumer on the destructive path is `recheck::member_occupancy`, where `Unknown` is a refusal, reached only through `run_all`.

...
Cargo reports retain profiles, role directories, individual incremental/build-script groups, and identified test/example executables. Their sizes come from the existing folded directory measurements; Cargo annotation does not walk the build tree again. IDs are scoped to the canonical storage container, independently of project ownership. Trusted event coverage lets unchanged containers reuse these units. Only these units receive nested history; ordinary compiler files do not become report or history rows. Nested history does not inflate project totals. The small report cache is compressed; measurements and reverse deltas remain in Parquet.

...
Cargo build rows carry nested identification and history. IDs are scoped to the canonical storage container, independently of project ownership. Nodes distinguish logical size, allocated subtree size, and leaf-only de-duplicated physical charges; nested history does not inflate project totals. Event paths invalidate cached subtrees, including same-size changes and removed roots. Incomplete reads fail the observation rather than recording false deletions. The report cache is compressed.
